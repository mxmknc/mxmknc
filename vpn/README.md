...
С `--firmware` скрипт сначала ставит `linux-image-virtual` — облегчённый метапакет ядра
для виртуальных машин, который, в отличие от `linux-image-generic`, от прошивок не зависит.
Поэтому вместе с firmware уезжает только `linux-image-generic`, а обновления ядра продолжают
приходить через `virtual`. Перед удалением прогоняется `apt-get -s purge`, и скрипт
отказывается работать, если apt собрался тронуть установленное ядро или сам `linux-image-virtual`.

Ещё `slim.sh` прописывает `APT::Install-Recommends "false"`, так что прошивки не вернутся
следующим `apt upgrade` через рекомендованные зависимости.
